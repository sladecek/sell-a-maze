...
use crate::shapes::{ANGLE_2PI, Shapes};
...
impl Builder {
    ...
    fn addWallShape(&self, shapes: &mut Shapes, roomCntThisLayer: i32, r1: i32, r2: i32, phi1: i32, phi2: i32, id: i32) {
        ...
        shapes.add_inner_wall(id, self.mapPhiD(rPhi1 as f64), r1, self.mapPhiD(rPhi2 as f64), r2, -1, -1);
    }
    ...
    fn generateRadialWalls(&mut self, graph: &mut Graph, shapes: &mut Shapes) {
        for layer in 1.. self.layers {
            ...
            for j in   0..cnt {
                ...
                // 1
                let phi = (j + 1) % cnt;
                self.addWallShape(shapes, cnt, computeRadius(layer - 1), computeRadius(layer), phi, phi, id);
            }
        }
    }
